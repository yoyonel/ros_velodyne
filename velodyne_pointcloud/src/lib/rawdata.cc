...
#define VLP16_OFFSET_DATA_TIMESTAMP     0x04B0    // 0x04DA - 42 = 04B0

/**
   * @brief RawData::unpack_vlp16_timestamp
   * @param pkt
   * @return
   */
uint32_t RawData::unpack_vlp16_timestamp(const velodyne_msgs::VelodynePacket &pkt)
{
    // ----------
    // TIMESTAMP
    // ----------
    ...
    //          uint8_t b4;
    //      };
    //      const S *raw_reverse_timestamp = (const S *) &pkt.data[VLP16_OFFSET_DATA_TIMESTAMP];
    ////      S *raw_reverse_timestamp = new S();
    ////      raw_reverse_timestamp->b1 = 0x61;
    ////      raw_reverse_timestamp->b2 = 0x67;
    ////      raw_reverse_timestamp->b3 = 0xB9;
    ////      raw_reverse_timestamp->b4 = 0x5A;

    ////      const S raw_timestamp = {
    ////          raw_reverse_timestamp->b4,
    ////          raw_reverse_timestamp->b3,
    ////          raw_reverse_timestamp->b2,
    ////          raw_reverse_timestamp->b1
    ////      };
    ////      const uint32_t *timestamp = (const uint32_t*)(&raw_timestamp.b1);
    //      // ----------

    ////      return *timestamp;
    //      return *(uint32_t*)(&raw_reverse_timestamp->b1);
    ////      return 0x5AB96761;

    return *(uint32_t*)(&pkt.data[VLP16_OFFSET_DATA_TIMESTAMP]);
}

const uint32_t RawData::compute_timeoffset(
        const uint8_t &_block,
        const uint8_t &_firing,
        const uint8_t &_dsr
        )
{
    // ----------
    // TIMESTAMP
    // ----------
    // _block: [0, BLOCKS_PER_PACKET[ = [0, 11]
    // _firing: [0, VLP16_FIRINGS_PER_BLOCK[ = [0, 1]
    // _dsr: [0, VLP16_SCANS_PER_FIRING[ = [0, 15]
    const uint8_t sequence_index = _block + _firing*BLOCKS_PER_PACKET;  // 0-23
    const uint8_t& data_point_index = _dsr;     // 0-15
    return (VLP16_FIRING_TOFFSET * sequence_index) +
            (VLP16_DSR_TOFFSET * data_point_index);
}
                           ...
                           VPointCloud &pc)
{
    ...
    // TIMESTAMP
    // ----------
    const uint32_t timestamp = this->unpack_vlp16_timestamp(pkt);
    //    ROS_INFO("timestamp: %" PRIu32 "", timestamp);
    ROS_INFO_THROTTLE(1, "timestamp: %" PRIu32 "", timestamp);
    // ----------
        ...
        for (int firing=0, k=0; firing < VLP16_FIRINGS_PER_BLOCK; firing++){
             ...
             */
                    ...
                    if (pointInRange(distance)) {
                        ...
                        //
                        const uint32_t exact_point_time = timestamp + compute_timeoffset(block, firing, dsr);
                        point.timestamp = exact_point_time;
                        //
                        ROS_INFO_THROTTLE(0.1, "Exact Point Time: %" PRIu32 "", exact_point_time);
                        // ----------
